...
int main(int argc, char *argv[]) {
    ...
    while (ctr < A_num_rows) {
        ...
        while (1) {
            blockCtr += nBlocks;
            ctr += A_ell_blocksize;
            if(( blockCtr >= MINIMUM_BLOCKS) || (ctr >= A_num_rows))
                break;
        }
        if (ctr >= A_num_rows) {
    ...
    std::cout << "\tTotal Blocks: " << total_blocks << "\tTime (seconds): " << searchTime << "\tPerf: " << tflops_bell << "\tGhost Blocks: " << totalGhostBlocks << " ( " << (double)totalGhostBlocks/(double)total_blocks*(double)100 << " %)" <<std::endl;
    ...
    __half *hC[k];
    for (int i=0; i<k; i++){
	hC[i] = new __half[(long int) 16 * B_num_cols * sizeof(__half)];
    	CHECK_CUDA( cudaMemcpy(hC[i], dC[i], (long int) 16 * B_num_cols * sizeof(__half),
